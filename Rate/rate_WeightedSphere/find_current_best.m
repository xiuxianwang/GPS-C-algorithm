%find function best
function [g_star,x_star] = find_current_best(model,A,Bpred,Z,x_star_last,sigma)
...
if gammaP == 2
    distxpred =  abs(repmat(reshape(x', [1 d K]),[k,1,1]) ...
        - repmat(A,[1 1 K])).^2;
else
    distxpred =  abs(repmat(reshape(x', [1 d K]),[k,1,1]) ...
        - repmat(A,[1 1 K]));
...
x_star = fmincon(@fun1,x_star_last,[],[],[],[],-5.12*ones(d,1),5.12*ones(d,1));
g_star = -fun1(x_star);
end